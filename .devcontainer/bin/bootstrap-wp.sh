...
log "Writing VS Code PHP debug configuration..."
DATE_UTC="${DATE_UTC:-$(date -u +'%Y-%m-%dT%H:%M:%SZ')}"
mkdir -p "$WORKSPACE/.vscode"
cat > "$WORKSPACE/.vscode/launch.json" <<JSON
{
  // Auto-generated by .devcontainer/bin/bootstrap-wp.sh
  // Generated at: ${DATE_UTC}
  // Source plugin slug: ${PLUGIN_SLUG}
  // Source plugin dir: ${PLUGIN_DIR}
  // Notes:
  // - Do not edit this file manually; changes will be overwritten on next start.
  // - If you need custom mappings, add them in a separate configuration and
  //   set an env flag (e.g., DISABLE_AUTOGEN_LAUNCH=1) to skip generation.

  "version": "0.2.0",
  "configurations": [
    {
      "name": "Listen for Xdebug (HTTP)",
      "type": "php",
      "request": "launch",
      "port": 9003,
      "log": true,
      "pathMappings": {
        "/var/www/html/wp-content/plugins/$PLUGIN_SLUG": "\${workspaceFolder}/$PLUGIN_DIR"
      }
    },
    {
