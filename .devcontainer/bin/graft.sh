...
apply_devcontainer() {
  ...
  local excludes=(--exclude "tmp/" --exclude "var/" --exclude "*.local" --exclude "*.local.*" --exclude ".cs_env.d/graft.env")
...
record_provenance_file() {
  ...
  cat > "$target_dir/.devcontainer/.cs_env.d/graft.env" <<EOF
...
SCION_ID=${scion_canonical}
SCION_REF=${scion_ref}
SCION_COMMIT=${scion_commit}
SCION_INSTALLED_AT=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
EOF
  info "Scion provenance recorded at $target_dir/.devcontainer/.cs_env.d/graft.env"
}
...
if [ "$VERB" = "upgrade" ]; then
  ...
  if [ -z "$SCION_SPEC" ]; then
    if [ -f ".devcontainer/.cs_env.d/graft.env" ]; then
      ...
      err "No .devcontainer/.cs_env.d/graft.env; supply --scion"
    fi
  fi
  [ -z "$TMP_BASE" ] && TMP_BASE="${TMP_BASE_DEFAULT:-/workspaces}"
fi
